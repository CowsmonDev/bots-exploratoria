...
class Conneccion:
    ...
    def __init__(self):
        self.__conn = mysql.connector.connect(host=Conneccion.__db_host, username=Conneccion.__db_user,
                                              password=Conneccion.__db_password, database=Conneccion.__db_database)
...
class PersonasDB(Conneccion):
    ...
    def existe_persona(self, id_conversacion):
        res = Conneccion.commit(self,
                                self.get_select() + " WHERE " + self.__id_conversacion + " = " + str(id_conversacion))
        return res

    def agregar_persona(self, datos):
        Conneccion.commit(self,
                          f"INSERT INTO {self.__table} ({self.get_campos()}) VALUES ({datos[0]}, {datos[1]}, {datos[2]}, {datos[3]})")
